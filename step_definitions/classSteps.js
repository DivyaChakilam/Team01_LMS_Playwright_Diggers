...
When('Admin clicks on save button without entering data', async ({classpagefixture}) => {
   await classpagefixture.clickSaveBtn();
});
...
When('Admin clicks on save button only by entering {string}', async ({classpagefixture}, ScenarioName) => {
    await classpagefixture.fillDetails(ScenarioName);
    await classpagefixture.clickSaveBtn();
});

Then('Admin gets message {string}', async ({classpagefixture}, expMessage) => {
  const actualMessage = await classpagefixture.gettoastMessage();
  await expect(actualMessage).toEqual(expMessage);
});

When('Admin enters {string} into the fields', async ({classpagefixture}, ScenarioName) => {
  await classpagefixture.fillDetails(ScenarioName);
  await classpagefixture.clickSaveBtn();
});

Then('Admin gets error message and class is not created', async ({classpagefixture}) => {
  await classpagefixture.gettoastMessage();
});
...
Then('Admin gets message {string} and see the updated values in data table', async ({classpagefixture}, expMessage) => {
  const actualMessage = await classpagefixture.getUpdateMessage();
  await expect(actualMessage).toEqual(expMessage);
});

Then('Admin should get Error message', async ({classpagefixture}) => {
  await classpagefixture.gettoastMessage();
});
...
Then('Admin gets a message {string} alert and do not see that Class in the data table', async ({classpagefixture}, expMessage) => {
  const actualMessage = await classpagefixture.getUpdateMessage();
  await expect(actualMessage).toEqual(expMessage);
});
...
When('Admin clicks {string} link on the {string} table', async ({commonpagefixture}, pageLink, module) => {
   // Get first row data before clicking page link
    firstRowBeforeClick = await commonpagefixture.getFirstRow();
  switch (pageLink) {
		case "Next page":
    ...
    // Get first row data after clicking page link
    firstRowAfterClick = await commonpagefixture.getFirstRow();
});
...
Then('Admin should see the next page record on the table  with Pagination has previous active link enabled', async ({commonpagefixture}) => {
  const previousPage = await commonpagefixture.getpreviousPageeLoc();
    ...
    expect(firstRowAfterClick).not.toBe(firstRowBeforeClick);
});
